        ...
        if (NdotL > 0.0)
        {
            if (lightCosSpotCutoff > 0.0)
            {
                ...
                if (spot > 0.0)
                {
                    //
                    lighted = true;
                    // First pass precomp ends here

                    vec3 diffuseContrib;
                    lambert(NdotL, materialDiffuse.rgb, lightDiffuse.rgb, diffuseContrib);
                    vec3 specularContrib;
                    specularCookTorrance(normal, eyeLightDir, eyeVector, materialShininess, materialSpecular.rgb, lightSpecular.rgb, specularContrib.rgb);
    ...
    out bool lighted)
{
    ...
    if (attenuation != 0.0)
    {
        ...
        if (NdotL > 0.0)
        {
            ...
            vec3 specularContrib;
            specularCookTorrance(normal, eyeLightDir, eyeVector, materialShininess, materialSpecular.rgb, lightSpecular.rgb, specularContrib.rgb);
            return vec4(lightAmbient.rgb*materialAmbient.rgb + attenuation*(diffuseContrib.rgb+specularContrib.rgb), 1.0);
    ...
    out bool lighted)
{
    ...
    if (NdotL > 0.0)
    {
        ...
        vec3 specularContrib;
        specularCookTorrance(normal, eyeLightDir, eyeVector, materialShininess, materialSpecular.rgb, lightSpecular.rgb, specularContrib.rgb);
        return vec4(lightAmbient.rgb*materialAmbient.rgb + diffuseContrib.rgb+specularContrib.rgb, 1.0);
    }
    return vec4(lightAmbient.rgb*materialAmbient.rgb, 1.0);
}
