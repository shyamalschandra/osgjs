...
module.exports = function(grunt) {
	...
	grunt.registerMultiTask('dir2json', 'Flatten a folder to a JSON file representing its contents', function() {
		...
		var options, key, root, dest, exclusions, includeExtension, path, result, contentsAreNumeric, removeExclusions, processFile, processDir, getKey;
		...
		if ( root.substr( -1 ) === options.pathSep ) {
		...
		getKey = function ( filepath ) {
			var key = filepath.split( options.pathSep ).slice( -1 )[0];

			if ( !includeExtension && key.lastIndexOf( '.' ) > 0 ) {
		...
		processDir = function ( dir, indent ) {
			...
			contents = grunt.file.expand( dir + options.pathSep + '*' ).filter( removeExclusions );
